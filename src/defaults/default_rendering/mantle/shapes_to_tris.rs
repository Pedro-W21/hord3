...
pub fn vec_to_complex(tris:&Vec<Triangle>, textures:&Vec<u32>, collluxes:&Vec<[(u8,u8,u8) ; 3]>, uvs:&Vec<[(f32,f32) ; 3]>, fields:&Vec<u32>) -> ApiLod {
    let mut out = ApiLod::new(Vec::new(), Vec::new(), Vec::new());
...
pub fn vec_to_complex_rand(tris:&Vec<Triangle>, texture_range:(u32,u32), bitflags:u32) -> ApiLod {
    let mut out = ApiLod::new(Vec::new(), Vec::new(), Vec::new());
...
pub fn cylinder_to_render_comp<const N:usize>(cylinder:&Cylinder<N>, side_texture:u32, top_texture:u32, bottom_texture:u32, reverse:bool, collux_bot:&Vec<(u8,u8,u8)>, collux_top:&Vec<(u8,u8,u8)>, top_field:u32, bot_field:u32, side_field:u32, bottom:bool, top:bool ) -> ApiLod {
    let mut lod_out = ApiLod::new(Vec::new(), Vec::new(), Vec::new());
    ...
    if bottom {
        ...
        let pre_len = lod_out.vertex_data.len();
        for i in 0..other.vertex_data.len() {
            lod_out.add_points([other.vertex_data[i].clone()]);
            
        }
        for i in 0..other.index_data.len() {
            lod_out.index_data.push(IndexData {vertex:other.index_data[i].vertex + pre_len as u32});
        }
    }
...
pub fn texture_collux_on_regular_face<const N:usize>(face:&FixedRegularFace<N>, texture_id:u32, collux:&Vec<(u8,u8,u8)>, reverse:bool, field:u32) -> ApiLod {
    let mut out = ApiLod::new(Vec::new(), Vec::new(), Vec::new());
    if N == 3 {
