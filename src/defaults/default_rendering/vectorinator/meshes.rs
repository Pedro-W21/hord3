...
impl MeshTriangles {
    ...
    pub fn get_triangle_uvs(&mut self, index:usize) -> ((usize,&mut f32, &mut f32), (usize,&mut f32, &mut f32), (usize,&mut f32, &mut f32)) {
        (
            (   
                self.p1.index[index],
                &mut self.p1.u[index],
...
impl TrianglePoints {
    ...
    pub fn get_point(&self, x:&Vec<f32>, y:&Vec<f32>, z:&Vec<f32>, index:usize) -> TrianglePointData {
        ...
        TrianglePointData::new(Vec3DfCam(Vec3Df::new(x[pos_index], y[pos_index], z[pos_index])), self.u[index], self.v[index], self.r[index], self.g[index], self.b[index])
    }
    pub fn get_point_no_data(&self, index:usize) -> TrianglePoint {
        TrianglePoint::new(self.index[index], self.u[index], self.v[index], self.r[index], self.g[index], self.b[index])
    }
}
