...
impl Textures {
    ...
    pub fn get_text_with_id(&self, id: u16) -> &DataTexture {
        &self.textures[self.sets[id as usize].set[self.sets[id as usize].act].id]
    }
    pub fn get_data_with_id(&self, id:&TextureSetID) -> &DataTexture {
        match id {
            TextureSetID::ID(ident) => self.get_text_with_id(*ident as u16),
            TextureSetID::Name(name) => match self.get_id_with_name(name) {
                Some(good_id) => match good_id { TextureSetID::ID(ident) => self.get_text_with_id(ident as u16), TextureSetID::Name(_) => panic!("couldn't get ident")}
