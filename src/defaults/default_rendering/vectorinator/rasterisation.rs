...
use super::{meshes::Rectangle, rendering_spaces::ViewportData, simd_geo::{SIMDVec2Df, SIMDVec3Df, LANE_COUNT, LANE_COUNT_I32, LANE_COUNT_U32}, textures::{argb_to_rgb, rgb_to_argb, MipMap, Textures}, triangles::{collux_f32_a_u8, collux_f32_to_u8_simd, collux_u8_a_f32, collux_u8_to_f32_simd, color_u32_to_u8_simd, simd_f32_to_u32_color, simd_rgb_to_argb, SingleFullTriangle}, VectorinatorRead};
...
impl<'a> InternalRasterisationData<'a> {
    pub fn from_vectorinator_read(read:&'a VectorinatorRead<'a>, viewport_data:ViewportData) -> Self {
        unsafe {
            Self { frambuf: &mut *read.framebuf.get_data_cell().get(), zbuf: &mut *read.zbuf.get(), dims: read.framebuf.get_dims(), textures: &read.textures, viewport_data}
        }
...
pub fn rasterise_if_possible<'a>(triangle:SingleFullTriangle, data:&mut InternalRasterisationData<'a>) {
    let area = triangle.get_area();
    if area > 0.0 {
        rasterise_any_collux(triangle, data, area);
    }
}

fn rasterise_any_collux<'a>(triangle:SingleFullTriangle, data:&mut InternalRasterisationData<'a>, area:HordeFloat) {
    let bounding_box = triangle.bounding_box(data.viewport_data.image_width, data.viewport_data.image_height);
    
    let x_dist_to_border = (data.dims.get_width() as i32).abs_diff(bounding_box.0.0.max(bounding_box.1.0)) as i32;
    
    if x_dist_to_border > LANE_COUNT_I32 || area > 50.0 {
        //full_normal_tri(triangle, data, area, bounding_box);
        full_simd_tri(triangle, data, area, bounding_box, x_dist_to_border as usize);
    }
    else {
        full_normal_tri(triangle, data, area, bounding_box);
    }
...
fn full_normal_tri<'a>(triangle:SingleFullTriangle, data:&mut InternalRasterisationData<'a>, area:HordeFloat, bounding_box:((i32, i32), (i32, i32))) {
    let texture = data.textures.get_text_with_id(triangle.texture_flags.0 as u16);
    let len = texture.get_mip_map(0).data.len();
    let mip_map = ((len as f32 / area).log2()) as u16;
    let t_len = texture.get_mip_map(mip_map as usize).data.len() - 1;
    
    let x_diff = bounding_box.0.0.abs_diff(bounding_box.1.0);
    let y_diff = bounding_box.0.1.abs_diff(bounding_box.1.1);
    let mip_map = texture.get_mip_map(mip_map as usize);
    let tri = TriangleData::from_tri_area(&triangle, area, mip_map);
    ...
    unsafe {
        let start_x_usize = point.x.to_int_unchecked::<usize>();
        let mut start_y = bounding_box.0.1 as usize * data.dims.get_width();
        for y in 0..y_diff {
            let mut pixel = (start_x_usize + start_y).min(data.zbuf.len() - 1);
            point.x = start_x;
            for x in 0..x_diff {
                let (w0, w1, w2, z, is_in) = tri.calc_w0_w1_w2_z_is_in(&point);                    
                if is_in && z < *data.zbuf.get_unchecked(pixel) {
                    *data.zbuf.get_unchecked_mut(pixel) = z;
                    ...
                    *data.frambuf.get_unchecked_mut(pixel) = final_color;
                }
                point.x += 1.0;
                pixel += 1;
            }
...
fn full_simd_tri<'a>(triangle:SingleFullTriangle, data:&mut InternalRasterisationData<'a>, area:HordeFloat, bounding_box:((i32, i32), (i32, i32)), dist_to_border:usize) {
    ...
    let mip_map = ((len / area).log2()) as usize;

    let mip_map = texture.get_mip_map(mip_map);
    let t_len = mip_map.len_m1 - 1;
    
    let addition_vector = Simd::splat(LANE_COUNT as f32);
    ...
    let tri = SIMDTriangleData::from_tri_area(&triangle, area, mip_map);
    let mut point = SIMDVec2Df::new(x_simd, Simd::splat(ys_f + 0.5));
    let t_len_simd = Simd::splat(t_len);
    //if x_diff > data.dims.get_width() as i32 || y_diff > data.dims.get_height() as i32 {
    //    dbg!(x_diff, y_diff);
    //}
    unsafe {
        let start_x_usize = point.x.to_int_unchecked::<usize>();
        let mut start_y = Simd::splat(ys_f as usize * data.dims.get_width());
        ...
        'fory : for y in 0..y_diff {
            ...
            for x in 0..x_simd_part {
                ...
                if in_mask.any() {
                    let zbuf_mask = Simd::gather_or(&data.zbuf, pixel, Simd::splat(INFINITY)).simd_ge(z);
                    was_inside = true;
                    if zbuf_mask.any() {
                        let final_mask:Mask<isize, LANE_COUNT> = ((zbuf_mask & in_mask)).into();
                        z.scatter_select_unchecked(data.zbuf, final_mask, pixel);
                        let (u, v) = tri.calc_xi_yi(w0, w1, w2, z);
                        let texture_pixel = collux_u8_to_f32_simd(color_u32_to_u8_simd(Simd::gather_or(&mip_map.data, (v.to_int_unchecked::<usize>() * tri.texture_width_usize + u.to_int_unchecked::<usize>()).simd_min(t_len_simd), Simd::splat(0))));
                        ...
                        final_color.scatter_select_unchecked(data.frambuf, final_mask, pixel);
                    }
...
struct SIMDTriangleData {
    ...
    collux:(Simd<f32, LANE_COUNT>, Simd<f32, LANE_COUNT>, Simd<f32, LANE_COUNT>),
...
impl SIMDTriangleData {
    pub fn from_tri_area(triangle:&SingleFullTriangle, area:HordeFloat, texture:&MipMap) -> Self {
        Self {
            ...
            collux:collux_u8_to_f32_simd((Simd::splat(triangle.p1.r), Simd::splat(triangle.p1.g), Simd::splat(triangle.p1.b))),
            area: Simd::splat(1.0/area),
...
pub fn textured_rect_at_depth<'a>(mut dims:Rectangle<i32>, depth:f32, texture:u16, data:&mut InternalRasterisationData<'a>, collux:(u8,u8,u8), viewport_data:&ViewportData) {
    if dims.width() > 0 && dims.height() > 0 {
        ...
        let data_text = data.textures.get_text_with_id(texture);
