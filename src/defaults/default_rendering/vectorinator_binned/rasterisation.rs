...
fn full_normal_tri<'a>(triangle:&SingleFullTriangle, data:&mut InternalRasterisationData<'a>, pre_data:&PreCalcdData, bounding_box:((i32, i32), (i32, i32)), bin:&Bin) {
    ...
    unsafe {
        let start_x_usize = bounding_box.0.0 as usize;
        ...
        for y in 0..y_diff {
            ...
            for x in 0..x_diff {
                ...
                if is_in && z < *data.zbuf.get_unchecked(pixel) {
                    ...
                    *data.nbuf.get_unchecked_mut(pixel) = pre_data.packed_normal;
            ...
            for x in 0..x_simd_part {
                ...
                if in_mask.any() {
                    ...
                    if zbuf_mask.any() {
                        ...
                        Simd::splat(pre_data.packed_normal).scatter_select_unchecked(data.nbuf, final_mask, pixel);
                        let (u, v) = tri.calc_xi_yi(w0, w1, w2, z);
